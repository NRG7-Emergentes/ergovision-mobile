...
class _ActivePauseState extends State<ActivePause> {
  ...
  Duration _timeUntilNextPause = const Duration(seconds: 30); // Changed to 30 seconds
  DateTime? _pauseStartTime;

  @override
  void initState() {
    super.initState();
    // Start countdown timer for next pause
      ...
      if (mounted) {
        setState(() {
          if (_timeUntilNextPause.inSeconds > 0) {
            ...
            _timeUntilNextPause = const Duration(seconds: 30); // Reset to 30 seconds
          }
        });
      }
    });
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<Map<String, dynamic>?>(
      valueListenable: NotificationListenerService().latestNotification,
      builder: (context, notification, child) {
        final bool hasPause = notification != null &&
          (notification['title']?.toString().toUpperCase().contains('PAUSED') ?? false);

        // Start or stop pause timer based on notification
        if (hasPause && _pauseStartTime == null) {
          final receivedAt = notification['_receivedAt'] as DateTime?;
          if (receivedAt != null) {
            _startPauseTimer(receivedAt);
          }
        } else if (!hasPause && _pauseStartTime != null) {
          _stopPauseTimer();
        }
                              ...
                              ? [
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(height: 10),
                                    // NEW: Elapsed pause time counter
                                    Center(
                                      child: Column(
                                        children: [
                                          Text(
